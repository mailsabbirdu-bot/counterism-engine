...
class RemotionRemaker:
    ...
    def gemini_change(self, scene_num: int, scene: Dict[str, Any]):
        ...
        for attempt in range(3):
            ...
            refine_prompt = (
                "TASK: OUTPUT RAW JSON ONLY. NO CONVERSATION.\n"
                "SYSTEM: PRODUCTION DATA REFINEMENT ENGINE.\n"
                f"REFINE SCENE: {scene_id}.\n"
                f"SOURCE NARRATION: {story_text}\n"
                f"CURRENT JSON: {current_scene_json}\n"
                "\nENGINE CONSTRAINTS:\n"
                "1. 3-COLUMN ANCHORS: ABSOLUTELY FORBIDDEN to center-stack. LEFT(x=400), CENTER(x=960), RIGHT(x=1520).\n"
                "2. VISUAL HIERARCHY: Eye-path must be clear. Primary, Secondary, Tertiary scale levels.\n"
                "3. PROGRESSIVE REVEAL: Stagger 'start' by 15-20f delay between items.\n"
                "4. DATA INTEGRITY: Use real numbers from story. NO placeholder values.\n"
                "5. INFOGRAPHIC STORIES: Use 'infographic_lines' (min 2) and 'groups' for procedural diagrams.\n"
                "6. VIDEO SAFE ZONES: Overlays must avoid center focal subject for video backgrounds.\n"
                "7. WHITESPACE: Maintain 300px between Primary elements. No overlaps.\n"
                ...
                "\nEXECUTION: OUTPUT UPDATED RAW JSON BLOCK NOW. NO PREAMBLE."
            )
