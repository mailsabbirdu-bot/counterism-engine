...
class RemotionRemaker:
    ...
    def gemini_change(self, scene_num: int, scene: Dict[str, Any]):
        ...
        for attempt in range(3):
            ...
            refine_prompt = (
                f"REFINE MOTION GRAPHICS JSON FOR SCENE: {scene_id}.\n"
                "\n--- SOURCE NARRATION ---\n"
                f"{story_text}\n"
                f"\nCURRENT JSON: {current_scene_json}\n"
                "\nACT AS A PROFESSIONAL MOTION ARCHITECT. RE-DESIGN THIS SCENE.\n"
                "COMPOSITION CONSTRAINTS (STRICT):\n"
                "1. 3-COLUMN ANCHORS: Absolutely NO center-stacking. LEFT(x=400), CENTER(x=960), RIGHT(x=1520).\n"
                "2. VISUAL HIERARCHY: Assign 'importance'. 1 Primary centerpiece, supporting nodes follow.\n"
                "3. PROGRESSIVE STORYTELLING: Stagger entrances (15-20f delay). NO simultaneous pops.\n"
                "4. DATA INTEGRITY: Use REAL NUMBERS from narration. NO placeholder values (10, 20, A, B).\n"
                "5. INFOGRAPHIC SYSTEMS: Procedural scenes MUST use 'infographic_lines' (min 2) and 'groups'.\n"
                "6. VIDEO SAFE-ZONES: Overlays MUST avoid center focal subjects for video backgrounds.\n"
                "7. WHITESPACE: Maintain 300px between Primary elements. Use 40/30/30 spatial balance.\n"
                "8. PERSISTENCE: duration = (scene_duration - start).\n"
                "\nVISUALS:\n"
                ...
                "\nTASK: OUTPUT UPDATED RAW JSON BLOCK NOW. NO CONVERSATION. NO PREAMBLE."
            )
