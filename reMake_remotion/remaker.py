...
class RemotionRemaker:
    ...
    def gemini_change(self, scene_num: int, scene: Dict[str, Any]):
        ...
        for attempt in range(3):
            ...
            refine_prompt = (
                f"YOU ARE A WORLD-CLASS DOCUMENTARY MOTION GRAPHICS DIRECTOR (Vox/Polymatter style).\n"
                f"REFINE THIS SCENE JSON FOR SCENE: {scene_id}.\n"
                f"NARRATION (MANDATORY SOURCE): {story_text}\n"
                f"TARGET LANGUAGE: {lang}\n"
                f"CURRENT JSON: {current_scene_json}\n"
                f"AVAILABLE FONTS: {target_fonts}\n"
                "\nSTRICT DIRECTOR RULES:\n"
                "1. 3-COLUMN ANCHORS: ABSOLUTELY FORBIDDEN to center-stack. spatial columns:\n"
                "   - LEFT (x=400): Text/Titles. CENTER (x=960): Primary Graphics. RIGHT (x=1520): Stats/KPIs.\n"
                "2. VISUAL HIERARCHY: Assign 'importance': 'primary' (largest focal point), 'secondary', 'tertiary'.\n"
                "3. PROGRESSIVE STORYTELLING: Stagger entrances (15-20f delay). Build the visual narrative progressively.\n"
                "4. NO PLACEHOLDERS: Use REAL DATA from narration. Visualize numbers accurately. NO 'INSIGHT' or 'DATA'.\n"
                "5. INFOGRAPHIC STORIES: Procedural scenes MUST use 'infographic_lines' and 'groups' to show relationships.\n"
                "6. VIDEO SAFE ZONES: If background_type='video', keep overlays to columns 1 and 3. Let video breathe.\n"
                "7. ASYMMETRICAL BALANCE: Maintain 200px whitespace. Let the composition breathe.\n"
                "8. PERSISTENCE: Overlays stay until scene changes. duration = (scene_duration - start).\n"
                "\nSCHEMA & VISUALS:\n"
                "- 'procedural_config': 'dark_particles', 'liquid_gradient', 'neon_grid'.\n"
                "- 'svg': (provider: lucide, query: icon_name, animation: draw|trace|pop, style: outline|tech).\n"
                ...
                f"\nFEEDBACK: {feedback_context if feedback_context else 'None'}\n"
                "\nOUTPUT RAW JSON BLOCK ONLY. NO EXPLANATIONS. NO ERROR MESSAGES."
            )
