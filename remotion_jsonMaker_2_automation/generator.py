...
class RemotionJsonMaker:
    ...
    def _harden_overlay_data(self, ov: Dict[str, Any], scene_context: str = "", scene_id: str = ""):
        ...
        if o_type == 'crve':
            for node in ov.get('nodes', []):
                if 'id' not in node:
                    import zlib
                    node['id'] = "node_" + str(zlib.adler32(str(node.get('label', '')).encode('utf-8')) & 0xffffffff)
                nlabel = str(node.get('label', ''))
                node_bn = any('\u0980' <= c <= '\u09FF' for c in nlabel)
                if 'font' not in node or not node['font']:
                    node['font'] = "Sohid_bangla" if node_bn else "Audiowide-Regular_english"
                if 'style_preset' not in node:
                    node['style_preset'] = "glass_disc"
                if 'scale' not in node:
                    node['scale'] = 1.0
                if 'importance' not in node:
                    node['importance'] = 1.0

    def _ensure_non_repeating_heading(self, text_ov, peer_ov):
        """Ensures that the heading text does not repeat the exact numerical values or labels displayed in peer_ov."""
        if not text_ov or not peer_ov: return
        heading = str(text_ov.get('content', '')).strip()
        peer_type = str(peer_ov.get('type')).lower()

        words_to_strip = []

        # 1. Peer is data visualization component
        if peer_type in ['indicator', 'shadcn_indicator', 'chart', 'shadcn_chart']:
            val = str(peer_ov.get('value', ''))
            lbl = str(peer_ov.get('label', ''))
            title = str(peer_ov.get('title', ''))

            if val: words_to_strip.extend(val.split())
            if lbl: words_to_strip.extend(lbl.split())
            if title: words_to_strip.extend(title.split())

        # 2. Peer is knowledge graph crve component
        elif peer_type == 'crve':
            # Extract labels of all nodes
            for node in peer_ov.get('nodes', []):
                nlbl = str(node.get('label', ''))
                if nlbl: words_to_strip.extend(nlbl.split())

        # Clean words_to_strip from punctuation and tiny connectors
        clean_strip = []
        for w in words_to_strip:
            cw = re.sub(r'[.।,!?"\'()]', '', w).strip()
            if len(cw) > 1 and cw not in ["একটি", "একজন", "এবং", "ও", "or", "and", "the"]:
                clean_strip.append(cw)

        # Strip from heading content
        heading_words = heading.split()
        cleaned_words = []
        for w in heading_words:
            cw = re.sub(r'[.।,!?"\'()]', '', w).strip()
            if cw in clean_strip:
                continue
            cleaned_words.append(w)

        final_heading = " ".join(cleaned_words).strip()

        # Fallback to pristine abstract headings if stripped completely or too short
        is_bn = any('\u0980' <= c <= '\u09FF' for c in heading)
        if len(final_heading) < 5:
            final_heading = "পরিদৃশ্য বিশ্লেষণ" if is_bn else "SCENE MATRIX"

        text_ov['content'] = final_heading
    ...
    def finalize_json_durations(self, data: Dict[str, Any], public_dir: str = "../public") -> Dict[str, Any]:
        ...
        for scene_idx, scene in enumerate(data['scenes']):
            ...
            opt = scene['visualization_option']
            print(f"   📊 Visualization Option: {opt}")
            ...
            for ov in raw_overlays:
                ...
                if not ov.get('id'): ov['id'] = f"ov_{id_num}_{len(processed_overlays)+1}"
                ...
                if ov['type'] == 'text':
                    ...
                    if not ov.get('hero_config'):
                        ...
                        if hero: ov['hero_config'] = {"word": hero['word'], "start": hero['start'], "color": self.MODERN_COLORS[(scene_idx + 2) % len(self.MODERN_COLORS)], "animation": ov.get('intro_animation', self.VALID_TEXT_ANIMS[scene_idx % len(self.VALID_TEXT_ANIMS)])}
                    else:
                        ...
                        if 'intro_animation' in ov:
                            h_conf['animation'] = ov['intro_animation']
                    ov['color'] = self.MODERN_COLORS[scene_idx % len(self.MODERN_COLORS)]
                    ov['fontSize'] = ov.get('fontSize', "120px")
                processed_overlays.append(ov)

            # Apply strict visual paradigm filtering
            text_ovs = [o for o in processed_overlays if str(o.get('type')).lower() == 'text']
            crve_ovs = [o for o in processed_overlays if str(o.get('type')).lower() == 'crve']
            viz_ovs = [o for o in processed_overlays if str(o.get('type')).lower() in ['chart', 'shadcn_chart', 'indicator', 'data_indicator', 'shadcn_indicator']]
            other_ovs = [o for o in processed_overlays if str(o.get('type')).lower() not in ['text', 'crve', 'chart', 'shadcn_chart', 'indicator', 'data_indicator', 'shadcn_indicator']]
            ...
            if opt == 'text':
                if text_ovs:
                    text_ov = text_ovs[0]
                    text_ov['importance'] = 'hero'
                    valid_overlays.append(text_ov)
                else:
                    ...
                    self._harden_overlay_data(text_ov, scene_context=story_text, scene_id=s_id)
                    valid_overlays.append(text_ov)
                valid_overlays.extend(other_ovs[:2]) # Keep a maximum of 2 ambient/shapes to be minimalistic

            elif opt == 'data_visualization':
                if viz_ovs:
                    valid_overlays.append(viz_ovs[0])
                else:
                    story_text = self.story_scenes.get(s_id, "STUDIO V4")
                    viz_ov = {
                        "id": f"ov_{id_num}_1",
                    ...
                    self._harden_overlay_data(viz_ov, scene_context=story_text, scene_id=s_id)
                    valid_overlays.append(viz_ov)

            elif opt == 'knowledge_graph':
                if crve_ovs:
            ...
            elif opt == 'text+data_visualization':
                text_ov = text_ovs[0] if text_ovs else None
                viz_ov = viz_ovs[0] if viz_ovs else None

                if not text_ov:
                    story_text = self.story_scenes.get(s_id, "Topic")
                    text_ov = {
                        "id": f"ov_{id_num}_heading",
                        "type": "text",
                        "content": story_text[:30],
                        "start": 15,
                        "duration": scene_duration - 45,
                ...
                text_ov['position'] = {"x": 960, "y": 320} # Center top safe heading position
                text_ov['fontSize'] = "80px"
                text_ov['importance'] = "secondary"

                self._ensure_non_repeating_heading(text_ov, viz_ov)

                valid_overlays.append(text_ov)
                valid_overlays.append(viz_ov)

            elif opt == 'text+knowledge_system':
                text_ov = text_ovs[0] if text_ovs else None
                crve_ov = crve_ovs[0] if crve_ovs else None
                ...
                self._ensure_non_repeating_heading(text_ov, crve_ov)

                valid_overlays.append(text_ov)
                valid_overlays.append(crve_ov)

            # Enforce graph referential integrity for crve type overlays
            for ov in valid_overlays:
                if str(ov.get('type')).lower() == 'crve':
                    node_ids = {n['id'] for n in ov.get('nodes', [])}
                    content_text = str(ov.get('content', ''))
                    is_bn_val = any('\u0980' <= c <= '\u09FF' for c in content_text)
                    valid_links = []
                    for link in ov.get('links', []):
                        src = link.get('source')
                        tgt = link.get('target')
                        if src not in node_ids:
                            ov['nodes'].append({
                                ...
                                "scale": 1.0,
                                "depth": 0,
                                "font": "Sohid_bangla" if is_bn_val else "Audiowide-Regular_english",
                                "style_preset": "glass_disc"
                            })
                            node_ids.add(src)
            ...
            if not scene['beats']:
                scene['beats'] = [{"frame": 0, "event": "scene_start"}]
            if 'connections' not in scene: scene['connections'] = []

            valid_ov_ids = {o['id'] for o in valid_overlays}
    ...
    def _interact_with_gemini(self, prompt: str, previous_json: str = None, errors: List[str] = None, score: int = 0, surgical_mode: bool = False, stubborn_issues: List[str] = None) -> str:
        # Create full prompt copy payload if previous json exists for robust iteration
        copy_payload = prompt
        if previous_json:
            error_summary = ""
        ...
        return self.get_automated_gemini_response(copy_payload)

    def get_automated_gemini_response(self, prompt_text):
        """
        ...
        print(f"[BRIDGE] Writing prompt to: {prompt_file}")
        with open(prompt_file, "w", encoding="utf-8") as f:
            f.write(prompt_text)

        print("[BRIDGE] Waiting for Android app to output reply.txt...")
    ...
    def generate(self, story: str, prompt_output_path: str = None, timestamp_context: str = None, scene_durations: List[int] = None, drive_prompt_path: str = None,
        ...
        full_prompt = (
            f"TASK: GENERATE A PRODUCTION-READY CINEMATIC KNOWLEDGE SYSTEM MANIFEST WITH DIVERSE PACING AND EXACTLY DESIGNED SCENE VISUALIZATIONS.\n\n"
            ...
            f"OBJECTIVE: Direct a high-end, clean, and ultra-minimalistic cinematic documentary where every scene is planned mathematically and matches one of five distinct visualization paradigms.\n\n"
            f"--- 3. MANDATORY SCENE VISUALIZATION OPTIONS ---\n"
            f"Every single scene MUST have exactly one `visualization_option` attribute from this registry. Design overlays strictly matching the option chosen:\n"
            f"1) \"text\": Only a single 'text' overlay with a high-fidelity 'hero_config' word-reveal animation is allowed in the scene overlays list. Use this for purely narrative, introductory, or transitional scenes.\n"
            f"2) \"data_visualization\": Only a single data visualization component ('indicator', 'shadcn_indicator', 'chart', or 'shadcn_chart') is allowed in the overlays list. Do not generate text overlays or headings.\n"
            f"3) \"knowledge_graph\": Only a single 'crve' overlay representing a node/link knowledge graph is allowed in the overlays list. Do not generate text overlays.\n"
            f"4) \"text+data_visualization\": Exactly two overlays allowed: 1) One 'text' overlay acting as a high-level centered header/title, positioned at C_TOP (960, 320), AND 2) One data visualization component (indicator/chart). IMPORTANT: The heading must NOT repeat the specific labels, values, or metrics shown inside the data viz component.\n"
            f"5) \"text+knowledge_system\": Exactly two overlays allowed: 1) One 'text' overlay acting as a high-level centered header/title, positioned at C_TOP (960, 320), AND 2) One 'crve' knowledge graph overlay. IMPORTANT: The heading must NOT repeat any node labels or relationships represented inside the graph.\n\n"
            f"--- 4. SCENE COGNITIVE MINIMALISM PROTOCOLS ---\n"
            f"- MINIMALIST SCENES: Scenes must never be crowded. Adhere to a strictly restricted count of overlays (1 overlay for options 1, 2, 3; exactly 2 overlays for options 4, 5).\n"
            f"- NON-REPETITIVE HEADINGS: When using options 4 and 5, the heading text MUST be an abstract, narrative-driven title (e.g. \"জনসংখ্যার চাপ\" or \"ঐতিহাসিক অর্জন\") and MUST NOT repeat the numeric values (like \"২২,৪০,০০,০০০+\" or \"৯৫%\") or entity node labels that the peer component itself displays.\n"
            f"- BANGLA MULTILINGUAL RULES: If the narration/story is in Bangla, headings, indicator labels, values, and knowledge graph nodes MUST be written in immaculate Bangla text and numerals (e.g., '২,২৪,০০,০০০+', '৯৫%', '৪০,০০০').\n\n"
            f"--- 5. SCHEMA AND COMPONENT AUTHORITY ---\n"
            f"- DATA VISUALIZATION COMPONENTS: Choose from Bklit UI templates: 'metric_tile', 'tech_badge', 'activity_ring', 'crypto_card', 'step_indicator_glass', 'glass_area', 'neon_bar', 'kpiNumber', etc. (See Variants Registry).\n"
            f"- KNOWLEDGE GRAPH 'crve' COMPONENTS: A single overlay of type \"crve\" containing precise arrays of 'nodes' and 'links'.\n"
            f"  - Each Node: {{\"id\": \"unique_id\", \"label\": \"Node Label\", \"type\": \"concept|hero|metric|location|event\", \"importance\": 1.0, \"scale\": 1.0, \"depth\": 0, \"font\": \"Sohid_bangla\", \"style_preset\": \"glass_disc\"}}\n"
            f"  - Each Link: {{\"id\": \"source_target\", \"source\": \"source_id\", \"target\": \"target_id\", \"relationship\": \"causes|triggers|leads_to|depends_on|supports|threatens\", \"strength\": 1.0}}\n"
            f"  - IMPORTANT: Every source and target inside the 'links' list MUST exist as a valid node inside the 'nodes' list to prevent simulation crashes!\n\n"
            ...
            f"CONNECTORS (RELATIONSHIPS): causes, leads_to, depends_on, located_in, transforms_into, increases, decreases, supports, threatens, flows_to, triggers, influences, smooth_curve, soft_arc, straight_flow, energy_flow, signal_beam, data_stream, s_curve, zigzag_soft, multi_branch, network_web, callout_line, camera_focus, timeline_path, route_path, curved_route, neon_connector, blueprint_connector, organic_connector.\n\n"
            f"--- 7. MANDATORY OUTPUT STRUCTURE ---\n"
            f"Output must be structured as valid JSON under the key \"scenes\". Each scene dictionary MUST contain:\n"
            f" - \"scene_id\": \"SCENE_X\" (e.g. \"SCENE_1\")\n"
            f" - \"visualization_option\": \"text\" | \"data_visualization\" | \"knowledge_graph\" | \"text+data_visualization\" | \"text+knowledge_system\"\n"
            f" - \"duration_in_frames\": 300\n"
            f" - \"background\": {{\"background_type\": \"procedural\", \"procedural_config\": {{\"variant\": \"neon_grid\"}}}}\n"
            f" - \"overlays\": [ ... ] (conforming strictly to the selected visualization_option)\n"
            f" - \"camera\": {{\"enabled\": true, \"shots\": [...]}}\n\n"
            f"FIRST, provide a <VISUAL_PLAN> block detailing the chosen visualization option per scene, reasoning, layout, theme, and transitions.\n"
